...
MAX_CONNECTIONS = 100 # Maximum no of parellel allowed connections.
        ...
        while True:
            ...
            for fileno, event in events:
                if fileno == serverSock.fileno():
                    try:
                        while True:
                            client_socket, _ = serverSock.accept() # Accpets the new connections.
                    ...
                    if len(responses[fileno]) == bytes_written:
                        ...
                        connections[fileno].shutdown(socket.SHUT_RDWR) # Shutdown the connection.
    finally:
        epoll.unregister(serverSock.fileno()) # closes the sever connections on exit.
        epoll.close()
        serverSock.close()

# Parse the HTTP request, identifies the request, and generates an valid response.
